!(function(window, document) {
    ...
    var $ = (function(document, s_addEventListener, s_querySelectorAll) {
        ...
        $.fn.scroll = function scroll(options) {
            ...
            var defaults = Object.assign({
                ...
                offset: 2,
                rowHeight: 48,
                scrollable: '.action-picker-ul',
                onChange: noop,
            }, options);
            defaults.bodyHeight = 5 * defaults.rowHeight;
            var items = defaults.items.map(function(item) {
                return '<div class="action-picker-li" style="height:' + defaults.rowHeight + 'px;">' + item.label + '</div>';
            }).join('');
            var $this = $(this);
            var obj = defaults.items[0];
            $this.find('.action-picker-ul').html(items);
            obj.sub && $this.find('.action-picker-unit').html(obj.sub).addClass(obj.cls);
            ...
            var currItemLight = function(index) {
                $scrollable.find('.action-picker-li').removeClass('action-picker-li-light').eq(index).addClass('action-picker-li-light');
            }
            ...
            function _move(pageY) {
                ...
                var rowHeight = defaults.rowHeight;
                var len = defaults.items.length
                var max = getMax(offset, rowHeight) + rowHeight;
                var min = getMin(offset, rowHeight, len) - rowHeight;
                var index = Math.round(offset - posY / rowHeight);
                if (posY > max) {
                    posY = max;
                } else if (posY < min) {
                    posY = min;
    ...
    function picker(items, options) {
        ...
        var result = [];
        var lineTemp = [];
        var $picker = $('<div class="action-picker-bd"></div>');
        ...
        function scroll(items, level) {
            ...
            $picker.find('.action-picker-group').eq(level).scroll({
                items: items,
                temp: lineTemp[level],
        ...
        var groupHtml = '<div class=action-picker-group> <div class=action-picker-ul></div> <div class=action-picker-unit></div> </div>';
        var groups = '';
        while (depth--) {
            groups += groupHtml;
        }
        groups += '<div class="action-picker-shadow"></div><div class="action-picker-indicator"></div>';
        $picker.html(groups);
    ...
    function datePicker(options) {
        ...
        for (var year = start.year; year <= end.year; year++) {
            ...
            var start_month = curr_year ? 1 : start.month;
            for (var month = start_month; month <= max_month; month++) {
                ...
                var start_date = curr_year && month == start_month ? start.date : 1;
                for (var date = start_date; date <= max_date; date++) {
                    var item_2 = {
                        sub: '日',
                        label: date,
                        value: date,
    ...
    function appointment(options) {
        ...
        for (var year = start.year; year <= end.year; year++) {
            ...
            var start_year = year == start.year;
            var end_year = year == end.year;

            var start_month = start_year ? start.month : 1;
            var max_month = end_year ? end.month : 12;
            for (var month = start_month; month <= max_month; month++) {
                var item_1 = {
                    sub: '月',
                    label: month,
                ...
                var end_month = end_year && month == end.month;
                var max_date = defaults.dd ? end_month ? end.date : new Date(year, month, 0).getDate() : 0;
                var start_date = start_year && month == start_month ? start.date : 1;
                for (var date = start_date; date <= max_date; date++) {
                    var item_2 = {
                        sub: '日',
                        label: date,
